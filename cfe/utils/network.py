...
class MetaConv(MetaModule):
    ...
    .. [1] Finn C., Abbeel P., and Levine, S. (2017). Model-Agnostic Meta-Learning
           ...
           Machine Learning (ICML) (https://arxiv.org/abs/1703.03400)
    """
    def __init__(self, in_channels, num_classes=5, hidden_size=64, feature_size=64, mlp=False):
        super(MetaConv, self).__init__()
        self.in_channels = in_channels
        ...
        if not mlp:
            ...
            self.fc = nn.Sequential(nn.Linear(feature_size, feature_size), nn.ReLU(), nn.Linear(feature_size, num_classes, bias=True))
...
def ModelConvOmniglot(num_classes, hidden_size=64):
    return MetaConv(1, num_classes, hidden_size=hidden_size,
                         feature_size=hidden_size)

def ModelConvOmniglotMLP(num_classes, hidden_size=64):
    return MetaConv(1, num_classes, hidden_size=hidden_size,
                         feature_size=hidden_size, mlp=True)

def ModelConvMiniImagenet(num_classes, hidden_size=64):
    return MetaConv(3, num_classes, hidden_size=hidden_size,
