...
class EpisodicOmniglot(EpisodicDataset):
    tasks_type = "clss"
    name = "omniglot"
    episodic=True
    ...
    def __init__(self, data_root, split, sampler, size, transforms):
        ...
        """
        self.data_root = os.path.join(data_root, "omniglot_64_K_500_%s.npz")
    ...
    def sample_images(self, indices):
        return np.transpose(self.features[indices], (0, 2, 3, 1))
...
if __name__ == '__main__':
    ...
    dataset = EpisodicOmniglot('../../data/encodings', 'train', sampler, 1000, transforms)
